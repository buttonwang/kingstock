"""66大顺 V1.6 回测脚本 — 三阶信号融合方案

V1.0 + V1.1 理念融合:
- 阶A: V1.0 Core (MACD严格金叉∩ZJTJ + 增强≥1 + Pure Hold)
- 阶B: 延续金叉扩容 (MACD延续金叉∩ZJTJ + 增强≥1 + 动态退出)
- 阶C: ML门控弹性扩容 (ZJTJ单过滤 + ML≥13 + 增强≥2 + 紧退出)
...
from src.indicators.macd import calculate_macd, is_macd_buy_signal, is_macd_continuation_signal
...
from scripts.v1_6_execution import simulate_v16_portfolio, V1_6_CONFIG

start_date, end_date = "20230601", "20260603"
print("=" * 80)
print("66大顺 V1.6 三阶信号融合回测")
print("=" * 80)
print(f"\n配置摘要:")
print(f"  【阶A】MACD严格金叉∩ZJTJ + 增强≥1 → Pure Hold (5%仓位)")
print(f"  【阶B】MACD延续金叉∩ZJTJ + 增强≥1 → 动态退出 (4%仓位)")
print(f"  【阶C】ZJTJ单过滤 + ML≥13 + 增强≥2 → 紧退出 (2%仓位)")
print(f"  市场门控: 强市→三阶全开 | 震荡→仅A阶半仓 | 弱市→不交易")
print()
...
results_v16 = []   # V1.6 signals with tier
results_v10 = []   # V1.0 signals (for comparison)

T0_ENHANCED_MIN = V1_6_CONFIG['tier_a']['enhanced_rules_min']
T1_ENHANCED_MIN = V1_6_CONFIG['tier_b']['enhanced_rules_min']
T2_ENHANCED_MIN = V1_6_CONFIG['tier_c']['enhanced_rules_min']
T0_ML_MIN = V1_6_CONFIG['tier_a']['ml_min']
T1_ML_MIN = V1_6_CONFIG['tier_b']['ml_min']
T2_ML_MIN = V1_6_CONFIG['tier_c']['ml_min']
...
for di, date_str in enumerate(trading_dates):
    ...
    for code in stock_dict:
        ...
        macd_continue = is_macd_continuation_signal(df_macd)
        ...
        if (macd_strict and in_zjtj
                and rules_passed >= T0_ENHANCED_MIN
                and weekly_ok
                and (ml_val is not None and ml_val >= T0_ML_MIN)):
            try:
                dk = calculate_kdj(df)
            ...
            continue  # 已分配到Tier A，跳过B/C

        # ── Tier B: 延续金叉扩容 ──
        if (not macd_strict and macd_continue and in_zjtj
                and rules_passed >= T1_ENHANCED_MIN
                and (ml_val is not None and ml_val >= T1_ML_MIN)):
            try:
                dk = calculate_kdj(df)
                dz = calculate_zjtj(df)
                scores = compute_total_score(df_macd, dk, dz, rps_rank, ml_score=ml_val)
            ...
            continue  # 已分配到Tier B，跳过C

        # ── Tier C: ML门控弹性扩容 ──
        if (in_zjtj  # ZJTJ单过滤（同V1.1）
                and rules_passed >= T2_ENHANCED_MIN
                and (ml_val is not None and ml_val >= T2_ML_MIN)
                and rps_rank <= 5):  # RPS仅前5板块
            try:
                dk = calculate_kdj(df)
                dz = calculate_zjtj(df)
                scores = compute_total_score(df_macd, dk, dz, rps_rank, ml_score=ml_val)
...
df_v16 = pd.DataFrame(results_v16)
df_v10 = pd.DataFrame(results_v10)

print(f"\nV1.6 信号总数: {len(df_v16)}")
if not df_v16.empty:
    tier_counts = df_v16['signal_tier'].value_counts().sort_index()
    for t, c in tier_counts.items():
        tier_name = {0: "A(V1.0核心)", 1: "B(延续金叉)", 2: "C(ML门控)"}
        print(f"  {tier_name.get(t, f'Tier{t}')}: {c}")
    print(f"  score_ml: mean={df_v16['score_ml'].mean():.1f}, min={df_v16['score_ml'].min():.1f}")
    print(f"  market_state: {df_v16['market_state'].value_counts().to_dict()}")
    df_v16.to_csv("data/output/v1_6_signals_debug.csv", index=False, encoding="utf-8-sig")
...
# ── V1.6 组合模拟（三阶差异化执行） ──
print(f"\n开始 V1.6 组合模拟...")
result_v16 = simulate_v16_portfolio(df_v16, stock_daily, trading_dates)
...
nav_v16 = build_nav_series(result_v16["daily_nav"], result_v16.get("all_dates", trading_dates))
nav_v10 = build_nav_series(result_v10["daily_nav"], trading_dates)

print(f"\n{'='*80}")
print("V1.6 三阶信号融合 - 回测结果")
print(f"{'='*80}")
print(f"{'指标':<20} {'V1.0':<14} {'V1.6(本版)':<14}")
print(f"{'-'*48}")
v10_tr = f"{result_v10.get('total_return', 0):.2f}%"
v16_tr = f"{result_v16.get('total_return', 0):.2f}%"
v10_ar = f"{result_v10.get('ann_return', 0):.2f}%"
v16_ar = f"{result_v16.get('ann_return', 0):.2f}%"
v10_sr = f"{result_v10.get('sharpe', 0):.2f}"
v16_sr = f"{result_v16.get('sharpe', 0):.2f}"
v10_dd = f"{result_v10.get('max_drawdown', 0):.2f}%"
v16_dd = f"{result_v16.get('max_drawdown', 0):.2f}%"
v10_wr = f"{result_v10.get('win_rate', 0):.1f}%"
v16_wr = f"{result_v16.get('win_rate', 0):.1f}%"
v10_pro = f"{result_v10.get('profit_ratio', 0):.2f}"
v16_pro = f"{result_v16.get('profit_ratio', 0):.2f}"
v10_pf = f"{result_v10.get('profit_factor', 0):.2f}"
v16_pf = f"{result_v16.get('profit_factor', 0):.2f}"
v10_tt = f"{result_v10.get('total_trades', 0)}"
v16_tt = f"{result_v16.get('total_trades', 0)}"
print(f"{'总收益率':<20} {v10_tr:<14} {v16_tr:<14}")
print(f"{'年化收益':<20} {v10_ar:<14} {v16_ar:<14}")
print(f"{'夏普比率':<20} {v10_sr:<14} {v16_sr:<14}")
print(f"{'最大回撤':<20} {v10_dd:<14} {v16_dd:<14}")
print(f"{'胜率':<20} {v10_wr:<14} {v16_wr:<14}")
print(f"{'盈亏比':<20} {v10_pro:<14} {v16_pro:<14}")
print(f"{'利润因子':<20} {v10_pf:<14} {v16_pf:<14}")
print(f"{'交易次数':<20} {v10_tt:<14} {v16_tt:<14}")

# 各tier统计
tier_stats = result_v16.get("tier_stats", {})
if tier_stats:
    print(f"\nV1.6 各层统计:")
    tier_names = {0: "A(V1.0核心)", 1: "B(延续金叉)", 2: "C(ML门控)"}
    for tid, stats in sorted(tier_stats.items()):
        print(f"  {tier_names.get(tid, f'Tier{tid}')}: "
              f"交易{stats['count']}次, "
              f"胜率{stats['win_rate']:.1f}%, "
              f"平均回报{stats['avg_return']:.2f}%")

exit_reasons = result_v16.get("exit_reasons", {})
if exit_reasons:
    print(f"\n退出原因分布:")
...
fig.suptitle("66大顺 V1.6 三阶信号融合回测曲线", fontsize=18, fontweight="bold")

# 1) 累计收益率
ax1 = axes[0]
if not nav_v16.empty:
    ax1.plot(nav_v16.index, (nav_v16.values - 1) * 100,
             label=f"66大顺 V1.6 ({v16_ar}年化 / {v16_dd}回撤)",
...
for name, ts in bench_norm.items():
    common = (nav_v16.index if not nav_v16.empty else nav_v10.index).tz_localize(None).intersection(ts.index)
    if len(common) > 0:
...
ax1.set_title("累计收益率曲线对比 (V1.6 vs V1.0)", fontsize=14, fontweight="bold")
...
if not nav_v16.empty:
    peak = nav_v16.cummax()
    dd = (nav_v16 - peak) / peak * 100
    ax2.fill_between(nav_v16.index, dd.values, 0, color=colors_v16, alpha=0.4,
                     label=f"V1.6 最大回撤 {v16_dd}")
if not nav_v10.empty:
    peak10 = nav_v10.cummax()
    dd10 = (nav_v10 - peak10) / peak10 * 100
    ax2.fill_between(nav_v10.index, dd10.values, 0, color=colors_v10, alpha=0.3,
...
ax3 = axes[2]
window = 63
for nav_s, name, clr in [(nav_v16, "V1.6", "#0066CC"), (nav_v10, "V1.0", "#FF6B35")]:
...
output_path = os.path.join("data", "output", "66dashun_v1_6_curve.png")
...
# ── 最终总结 ──
print(f"\n{'='*80}")
print("66大顺 V1.6 三阶信号融合 — 总结")
print(f"{'='*80}")
print(f"  V1.0: {len(df_v10)}信号, {v10_ar}年化, {v10_dd}回撤, 夏普{v10_sr}")
print(f"  V1.6: {len(df_v16)}信号, {v16_ar}年化, {v16_dd}回撤, 夏普{v16_sr}")
target_hit = ""
ann = result_v16.get("ann_return", 0)
dd = result_v16.get("max_drawdown", 0)
if ann >= 30 and dd <= 15:
    target_hit = "  【目标达成】年化≥30% 且 回撤≤15%  ✓"
elif ann >= 15 and dd <= 10:
    target_hit = "  【接近目标】改良显著但需进一步优化"
else:
