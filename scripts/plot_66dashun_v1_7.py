"""66大顺 V1.7 回测脚本 — 精简双阶方案

强化V1.0核心（放松门控）+ 双阶风控:
- 阶A: V1.0 Core放松 (MACD严格金叉∩ZJTJ + 周线 + ML≥8 + Pure Hold)
- 阶B: ML门控弹性扩容 (ZJTJ单过滤 + ML≥13 + 增强≥2 + RPS≤5 + 紧退出)
...
from scripts.v1_7_execution import simulate_v17_portfolio, V1_7_CONFIG

start_date, end_date = "20230601", "20260603"
print("=" * 80)
print("66大顺 V1.7 精简双阶方案回测")
print("=" * 80)
print(f"\n配置摘要:")
print(f"  【阶A】MACD严格金叉∩ZJTJ + 周线 + ML≥6 → Pure Hold (5%仓位)")
print(f"  【阶B】ZJTJ单过滤 + ML≥13 + 增强≥2 + RPS≤5 → 紧退出 (5%仓位)")
print(f"  市场门控: 强市→双阶全开 | 震荡→A阶半仓+B阶半仓 | 弱市→不交易")
print()
...
results_v17 = []   # V1.7 signals with tier
results_v10 = []   # V1.0 signals (for comparison)

T0_ENHANCED_MIN = V1_7_CONFIG['tier_a']['enhanced_rules_min']  # 0
T1_ENHANCED_MIN = V1_7_CONFIG['tier_b']['enhanced_rules_min']  # 2
T0_ML_MIN = V1_7_CONFIG['tier_a']['ml_min']    # 8
T1_ML_MIN = V1_7_CONFIG['tier_b']['ml_min']    # 13
# V1.0 comparison uses original V1.0 params
V10_ML_MIN = 10
V10_ENHANCED_MIN = 1
...
for di, date_str in enumerate(trading_dates):
    ...
    for code in stock_dict:
        ...
        # ── Tier A: 放松的V1.0核心（V1.7）──
        if (macd_strict and in_zjtj
                and weekly_ok
                and (ml_val is not None and ml_val >= T0_ML_MIN)):  # ML≥8, 无需增强规则
            try:
                dk = calculate_kdj(df)
            ...
            # V1.0 同源信号：仅当满足原始V1.0条件时才加入
            if rules_passed >= V10_ENHANCED_MIN and (ml_val is not None and ml_val >= V10_ML_MIN):
                results_v10.append({
                    "date": date_str, "code": code, "name": stock_name_map.get(code, ""),
                    "score_ml": score_ml,
                    "total_score": scores.get("total_score", 0),
...
df_v17 = pd.DataFrame(results_v17)
df_v10 = pd.DataFrame(results_v10)

print(f"\nV1.7 信号总数: {len(df_v17)}")
if not df_v17.empty:
    tier_counts = df_v17['signal_tier'].value_counts().sort_index()
    for t, c in tier_counts.items():
        tier_name = {0: "A(放松V1.0核心)", 1: "B(ML门控扩容)"}
        print(f"  {tier_name.get(t, f'Tier{t}')}: {c}")
    print(f"  score_ml: mean={df_v17['score_ml'].mean():.1f}, min={df_v17['score_ml'].min():.1f}")
    print(f"  market_state: {df_v17['market_state'].value_counts().to_dict()}")
    df_v17.to_csv("data/output/v1_7_signals_debug.csv", index=False, encoding="utf-8-sig")
...
# ── V1.7 组合模拟（双阶差异化执行） ──
print(f"\n开始 V1.7 组合模拟...")
result_v17 = simulate_v17_portfolio(df_v17, stock_daily, trading_dates)
...
nav_v17 = build_nav_series(result_v17["daily_nav"], result_v17.get("all_dates", trading_dates))
nav_v10 = build_nav_series(result_v10["daily_nav"], trading_dates)

print(f"\n{'='*80}")
print("V1.7 精简双阶方案 - 回测结果")
print(f"{'='*80}")
print(f"{'指标':<20} {'V1.0':<14} {'V1.7(本版)':<14}")
print(f"{'-'*48}")
v10_tr = f"{result_v10.get('total_return', 0):.2f}%"
v17_tr = f"{result_v17.get('total_return', 0):.2f}%"
v10_ar = f"{result_v10.get('ann_return', 0):.2f}%"
v17_ar = f"{result_v17.get('ann_return', 0):.2f}%"
v10_sr = f"{result_v10.get('sharpe', 0):.2f}"
v17_sr = f"{result_v17.get('sharpe', 0):.2f}"
v10_dd = f"{result_v10.get('max_drawdown', 0):.2f}%"
v17_dd = f"{result_v17.get('max_drawdown', 0):.2f}%"
v10_wr = f"{result_v10.get('win_rate', 0):.1f}%"
v17_wr = f"{result_v17.get('win_rate', 0):.1f}%"
v10_pro = f"{result_v10.get('profit_ratio', 0):.2f}"
v17_pro = f"{result_v17.get('profit_ratio', 0):.2f}"
v10_pf = f"{result_v10.get('profit_factor', 0):.2f}"
v17_pf = f"{result_v17.get('profit_factor', 0):.2f}"
v10_tt = f"{result_v10.get('total_trades', 0)}"
v17_tt = f"{result_v17.get('total_trades', 0)}"
print(f"{'总收益率':<20} {v10_tr:<14} {v17_tr:<14}")
print(f"{'年化收益':<20} {v10_ar:<14} {v17_ar:<14}")
print(f"{'夏普比率':<20} {v10_sr:<14} {v17_sr:<14}")
print(f"{'最大回撤':<20} {v10_dd:<14} {v17_dd:<14}")
print(f"{'胜率':<20} {v10_wr:<14} {v17_wr:<14}")
print(f"{'盈亏比':<20} {v10_pro:<14} {v17_pro:<14}")
print(f"{'利润因子':<20} {v10_pf:<14} {v17_pf:<14}")
print(f"{'交易次数':<20} {v10_tt:<14} {v17_tt:<14}")

# 各tier统计
tier_stats = result_v17.get("tier_stats", {})
if tier_stats:
    print(f"\nV1.7 各层统计:")
    tier_names = {0: "A(放松V1.0核心)", 1: "B(ML门控扩容)"}
...
exit_reasons = result_v17.get("exit_reasons", {})
if exit_reasons:
    print(f"\n退出原因分布:")
...
fig.suptitle("66大顺 V1.7 精简双阶方案回测曲线", fontsize=18, fontweight="bold")

# 1) 累计收益率
ax1 = axes[0]
if not nav_v17.empty:
    ax1.plot(nav_v17.index, (nav_v17.values - 1) * 100,
             label=f"66大顺 V1.7 ({v17_ar}年化 / {v17_dd}回撤)",
...
for name, ts in bench_norm.items():
    common = (nav_v17.index if not nav_v17.empty else nav_v10.index).tz_localize(None).intersection(ts.index)
    if len(common) > 0:
...
ax1.set_title("累计收益率曲线对比 (V1.7 vs V1.0)", fontsize=14, fontweight="bold")

# 2) 回撤曲线
ax2 = axes[1]
if not nav_v17.empty:
    peak = nav_v17.cummax()
    dd = (nav_v17 - peak) / peak * 100
    ax2.fill_between(nav_v17.index, dd.values, 0, color="#0066CC", alpha=0.4,
                     label=f"V1.7 最大回撤 {v17_dd}")
if not nav_v10.empty:
...
ax3 = axes[2]
window = 63
for nav_s, name, clr in [(nav_v17, "V1.7", "#0066CC"), (nav_v10, "V1.0", "#FF6B35")]:
...
output_path = os.path.join("data", "output", "66dashun_v1_7_curve.png")
...
# ── 最终总结 ──
print(f"\n{'='*80}")
print("66大顺 V1.7 精简双阶方案 — 总结")
print(f"{'='*80}")
print(f"  V1.0: {len(df_v10)}信号, {v10_ar}年化, {v10_dd}回撤, 夏普{v10_sr}")
print(f"  V1.7: {len(df_v17)}信号, {v17_ar}年化, {v17_dd}回撤, 夏普{v17_sr}")
target_hit = ""
ann = result_v17.get("ann_return", 0)
dd = result_v17.get("max_drawdown", 0)
if ann >= 25 and dd <= 10:
