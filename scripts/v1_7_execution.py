"""66大顺 V1.7 执行层 — 双阶信号退出规则与组合模拟

Tier A: V1.0 Core 放松 — Pure Hold (7/5/3天)
Tier B: ML门控弹性扩容(原C层) — 紧移动止盈(8%/3%) + 硬止损(-6%) + 时间止损(4天)
...
# ── V1.7 配置 ──────────────────────────────────────────
V1_7_CONFIG = {
    # Tier A: V1.0 核心进一步放松（纯持有到期）
    'tier_a': {
        'ml_min': 6,
        'enhanced_rules_min': 0,
        'hold_days': {13: 7, 11: 5, 8: 3},
        'max_position_pct': 0.05,
    },
    # Tier B: ML门控弹性扩容（紧退出，原C层）
    'tier_b': {
        'ml_min': 13,
...
def check_tier_b_exit(pos: dict, current_price: float) -> str:
    """Tier B 退出检查：紧硬止损 + 紧移动止盈 + 时间止损"""
    cfg = V1_7_CONFIG['tier_b']
...
def get_tier_hold_days(ml_score: float, tier: int) -> int:
    """根据ML评分和tier获取持有天数"""
    if tier == 0:
        hold_map = V1_7_CONFIG['tier_a']['hold_days']
    elif tier == 1:
        hold_map = V1_7_CONFIG['tier_b']['hold_days']
    else:
        hold_map = {13: 7, 11: 5, 8: 3}
...
def get_tier_position_pct(tier: int) -> float:
    """获取tier对应的单仓最大比例"""
    if tier == 0:
        return V1_7_CONFIG['tier_a']['max_position_pct']
    elif tier == 1:
        return V1_7_CONFIG['tier_b']['max_position_pct']
    return 0.05


def get_market_multiplier(tier: int, market_state: str) -> float:
    """获取tier在某市场状态下的仓位乘数"""
    mult_map = V1_7_CONFIG['market_mult'].get(tier, {})
    return mult_map.get(market_state, 0.0)


def simulate_v17_portfolio(
    ...
    signals_df 必须包含 'signal_tier' 列 (0=Tier A, 1=Tier B)
    """
    if signals_df.empty:
    ...
    for date_str in all_dates:
        ...
        if today_signals_raw:
            ...
            for sig in tier0_selected + tier1_selected:
                code = sig["code"]
                if code in active:
                    continue
